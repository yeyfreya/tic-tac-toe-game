...
class GameLog:
    def __init__(self, log_directory='logs'):
        ...
        self.log_file = os.path.join(log_directory, 'game_logs.csv')
        self.fieldnames = ['game_id', 'start_time', 'end_time', 'duration', 'moves', 'winner']

    def log_game(self, game_id, start_time, end_time, duration, moves, winner):
...
class TicTacToe:
    ...
    def get_player_input(self):
        if self.current_player.is_human():
            prompt = f"Player {self.current_player.get_symbol()}, enter cell number (1-9) for game {self.game_id}:"
            try:
            ...
            print(f"Player {self.current_player.get_symbol()}'s move: {cell_number} for game {self.game_id}")
